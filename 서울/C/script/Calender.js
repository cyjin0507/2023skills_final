class Calender {
    ...
    async init() {
        this.data = await $.getJSON('/reserv/get')
    ...
    reservModal(e) {
        ...
        $('#reserv-date').html(e.target.dataset.date)
        $('#reserv-data-val').val(e.target.dataset.date)
        ...
        new Location(e.target.dataset.date)
    }
    ...
    draw() {
        let firstDate = new Date(this.nowMonth.getFullYear(), this.nowMonth.getMonth(), 1);     // 이번달 1일
            ...
            for (let nowDay = firstDate; nowDay <= lastDate; nowDay.setDate(nowDay.getDate() + 1)) {   // day는 날짜를 저장하는 변수, 이번달 마지막날까지 증가시키며 반복  
                ...
                let date = nowDay.getFullYear() + "-" + this.leftPad(nowDay.getMonth() + 1) + "-" + this.leftPad(nowDay.getDate())

                let left = this.data.filter(x=> x.reserv_date == date).length


                let nowColumn = nowRow.insertCell();        // 새 열을 추가하고
                nowColumn.innerHTML = `
                <div>${this.leftPad(nowDay.getDate())}</div>
                <button data-idx=${idx} data-date="${date}" class="btn btn-primary reserv-btn">예약가능</button>
                <span>${16 - left} / 16</span>
                `;      // 추가한 열에 날짜 입력
