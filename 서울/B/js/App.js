class App {
    ...
    addEvent() {
        ...
        $('#list tbody').click((e)=> {
            if(!e.target.className.includes('basket-btn')) return
            let product = e.target.dataset
            this.basketInsert(product)
        })

        $('#list tbody').click((e)=> {
            if(!e.target.className.includes('detail-btn')) return
            let product = e.target.dataset
    ...
    basketInsert(product) {
        if(!this.basket.find(x=>x.name==product.name)) {
            this.basket.push({
                "name" : product.name,
                "price" : product.price,
                "count" : 1
            ...
            this.basket.find(x=>x.name==product.name).count++
        }
        this.drawBasket()
        ...
        this.basket.forEach(x=> {
            $('#basket tbody').append(`
                <tr>
                    <td>${x.name}</td>
                    <td>${x.price}</td>
                    <td><input type="number" data-name="${x.name}" class="basket-count form-control" value="${x.count}" min="1"></td>
    ...
    basketCount() {
        $('.basket-count').on('change', (e)=> {
            ...
            this.basket.find(x=>x.name==product).count = value
    ...
    basketDelete() {
        $('.basket-delete').click((e)=> {
            let product = e.target.dataset.name
            this.basket.splice(this.basket.indexOf(this.basket.find(x=>x.name==product)),1)
            this.drawBasket()
        })
    }
