...
class Place {
    ...
    async getJson() {
        let jsonData;
        await $.ajax({
            url: '/getPlace?random=' + Math.random(),
            dataType: 'json',
            cache: "false",
    ...
    // html에 넣기
    setData() {
        this.jsonData['place'].forEach(x => {

            let reviewCnt = 0;
            let rate = 0;
            let rateCnt  = 0;
            this.jsonData['review'].forEach(i=>{
                if(i.title == x.title) {
                    reviewCnt++
                    rateCnt++
           ...
           let html = `
            <div class="card">
            <img src="./resources${x.image}" alt="" class="card-img-top">
            <div class="card-body">
                <h5 class="card-title">제목 : ${x.title}</h5>
                <p class="card-text">별점 : ${ rateCnt != 0? Math.round((rate/ rateCnt) * 10) / 10 :0 }</p>
                <p class="card-text">리뷰 수 : ${reviewCnt}</p>
                <p class="card-text">시내로부터의 거리 데이터 : ${x.distance}</p>
                <a href="/review?idx=${x.idx}&title=${x.title}" class="btn btn-primary">리뷰 작성 버튼</a>
            </div>
        </div>`
    ...
    getReviews() {
        ...
        this.jsonData['review'].forEach(x => {
            let html = `
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title"><b>이름 : ${x.title}</b></h5>
                    <p class="card-text">방문날짜 : ${x.visit}</p>
                    <p class="card-text">별점 : ${x.rate}</p>
...
class Review {
    ...
    init() {
        ...
        this.idxValue = document.querySelector('input[type="hidden"]')
...
class Game {
    ...
    addEvent() {
        this.snakeBtn.addEventListener('click', () => {
            if(this.name.value != "") {
                window.location.href = '/snake?name=' + this.name.value
        ...
        this.walnutBtn.addEventListener('click', () => {
            if(this.name.value != "") {
                window.location.href = '/walnut?name=' + this.name.value
